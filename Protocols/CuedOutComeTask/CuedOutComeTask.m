...
%CUEDOUTCOMETASK Cued outcome task protocol.
%   CUEDOUTCOMETASK is a Bpod (Sanworks) protocol controlling a probabilistic
%   pavlovian conditioning task. The protocol consist of two auditory cues
%   predicting likely reward (80% reward, 10% punishment, 10% omission) or
%   likely punishment (25% reward, 65% punishment, 10% omission).
%   Parameters for intermediate training stages can be set in the S struct.

%   See also FINDTEENSYPORT, OUTCOMEPLOT_PAVLOV and
...
if isempty(fieldnames(S))  % If settings file was an empty struct, populate struct with default settings
    ...
    S.GUI.DirectWater = 10; %How many times do the animals receive 'free water' during the inintial training (stage 1 and 2)
...
% Main trial lopp
for currentTrial = 1:S.GUI.DirectWater
...
%Main trial loop
for currentTrial =  (S.GUI.DirectWater+1):MaxTrials
    ...
    if ~isempty(fieldnames(RawEvents)) % If trial data was returned
        ...
        Outcomes = UpdateOutcomePlot(out, BpodSystem.Data, UsOutcome, S.GUI.DirectWater); % update outcome plot
...
% -------------------------------------------------------------------------
function Outcomes = UpdateOutcomePlot(out, Data, UsOutcome,n)
